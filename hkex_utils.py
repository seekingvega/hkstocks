...
from data_utils import timed_lru_cache, get_floats, str_to_date

def parse_hk_ticker(ticker:str, allow_invalid: bool = False):
	''' ensure tickers are in the 0001.HK format
	'''
	if '.HK' in ticker.upper() or ' HK' in ticker.upper():
		ticker = ticker.replace(' ','.')
		return ticker.split('.')[0].zfill(4) + '.HK'
	else:
		if allow_invalid:
			if not ticker.isnumeric():
				return None
		assert ticker.isnumeric(), 'ticker must be all numeric, or otherwise ends with "HK" (e.g. 0005.HK)'
